...
Процедура Удалить(Команда)
	Если ВыборКода = 0 И ВыборСтроки = 0 Тогда
		Сообщить("Введите код и номер строки");
	ИначеЕсли ВыборКода <> 0 И ВыборСтроки <> 0 Тогда
		Удаление(ВыборКода,ВыборСтроки);
	КонецЕсли;
КонецПроцедуры

&НаСервере
Функция Удаление(Айди,Выделенные)
	Попытка
	Элементы.Таблица1.ВыделенныеСтроки.Удалить(ВыборСтроки-1);
	ДанныеГраждан.Удалить(ВыборСтроки-1);
Исключение
	Сообщить("Ошибка!");
	КонецПопытки;
	...
	Справочники.Вопросы.НайтиПоКоду(Айди).ПолучитьОбъект().Удалить();
